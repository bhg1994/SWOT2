...
const FacilityrentalForStudy = () => {
    ...
    const { selectedRoom,studyReservationData } = useSelector(state => state.room);
    ...
    let names = userparser(studyReservationData.applications, studyReservationData.users);
    
    const [content, setContent] = useState("");
    const [visible, setVisible] = useState(false);
    const [maximum, setMaximum] = useState(studyReservationData.applications.length+1);
    const [currentphoneNo, setCurrentphoneNo] = useState("");
    const [phone, setPhone] = useState(0);

    const [reason, onChangeReason] = useInput(names+studyReservationData.boards.body);
